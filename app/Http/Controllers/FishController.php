...
class FishController extends Controller
{
    ...
    function intr(){
     ...
     function flist(){
        $fid=1;
        $loc=DB::select("SELECT u.id,u.name,u.contact,u.address from users as u
           Inner join entreprenuer_interest as p on p.entrepreneur_id=u.id
           where p.vendor_id=$fid
       order BY u.id");
    ...
    function elist(){
        ...
        $sub=DB::select("SELECT farmer_id from entreprenuer_interest 
        where entreprenuer_id=$id");
         $p=DB::select("SELECT pincode from users
         where id=$id");
         $pin=json_decode(json_encode($p),true) ;
        //print_r($pp);
    ...
    function entr(){
        ...
        $fid=request('fid');
        $eid=request('eid');
        DB::insert("INSERT into entreprenuer_interest (farmer_id,entreprenuer_id)
                    VALUES($fid,$eid)");
